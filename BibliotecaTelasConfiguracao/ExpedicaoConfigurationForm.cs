...
{
   public partial class ExpedicaoConfigurationForm : IWTBaseForm
   {
     ConfLoader ConfMngr;
     public ExpedicaoConfigurationForm(ConfLoader Conf)
        {
            InitializeComponent();
            this.ConfMngr = Conf;
      ...
      private void carregarImpressoras()
      {
          foreach (string strPrinter in PrinterSettings.InstalledPrinters)
          {
              ...
              cmbImpressoraPKL.Items.Add(strPrinter);
          }
          ...
          this.cmbImpressoraPKL.SelectedItem = IWTConfiguration.Conf.getConf(Constants.IMPRESSORA_OP);
      }

      private string salvarImpressoras()
      {
          string err = "";
          err += IWTConfiguration.Conf.setConf(Constants.INTERNAL_LABEL_PRINTER,
                                               cmbImpressoraInterna.SelectedItem.ToString(), "XML",
                                               LoginClass.UsuarioLogado.loggedUser.Login);
          ...
          err += IWTConfiguration.Conf.setConf(Constants.IMPRESSORA_OP, cmbImpressoraPKL.SelectedItem.ToString(), "XML", LoginClass.UsuarioLogado.loggedUser.Login);
      ...
      {
          if (this.ConfMngr.getConf(Constants.TIPO_ETIQUETA_EXPEDICAO)=="1")
          {
              this.groupBox1.Text = "Etiqueta Expedição (10,8 cm x 18,9 cm)";
          }
          else
          {
              this.groupBox1.Text = "Etiqueta Expedição (8,5 cm x 5 cm)";
          }
      }
   }
}
