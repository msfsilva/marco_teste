...
namespace BibliotecaValidacoes.O.IPI
{
    public class O01_10:IValidacao
    {
        public bool Validar(TNFe nfe, out string error)
        {
            if (nfe.infNFe.ide.mod == TMod.Item65)
            {
                ...
                {
                    if (item.imposto.Items.Any(a => a is TIpi))
                    {

                        error += "NFCe com grupo de IPI no produto " + item.prod.cProd + " / ";
                    }
