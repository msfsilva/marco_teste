...
{
    public partial class CadMaterialFiscalForm : IWTBaseForm
    {
        readonly long? id;
        bool loading = true;
        string idICMS="";
        public CadMaterialFiscalForm(long? id)
        {
            InitializeComponent();
            this.id = id;
        ...
        private void loadToolTips()
        {
            ...
            //toolTip2.SetToolTip(txtGtin, "Preencher com o código GTIN-8, GTIN-12, GTIN-13 ou GTIN-14 (antigos código EAN, UPC, DUN-14), não informar caso o produto não possua esse código.");
            toolTip3.SetToolTip(nudGenero, "Gênero do material. Preencher de acordo com a tabela de capítulos da NCM");
        }
        ...
        private void loadCombos()
        {
            try
            {
                ...
                string sql2 = "SELECT id_ncm, ncm_codigo|| ' - ' ||ncm_descricao as nome FROM public.ncm;";

                IWTPostgreNpgsqlAdapter adapter2 = new IWTPostgreNpgsqlAdapter(sql2, this.SingleConnection);
                if (adapter2 != null)
                {
                    ...
                    binding2.DataSource = ds2.Tables[0];
                    this.cmbNCM.ValueMember = "id_ncm";
                    this.cmbNCM.DisplayMember = "nome";
                    this.cmbNCM.DataSource = binding2;
                }
            }
            catch (Exception e)
            {
        ...
        private void loadEdit()
        {
            try
            {
                ...
                if (read.HasRows)
                {
                    ...
                    this.cmbOrigem.SelectedValue = read["mfs_origem"].ToString();
                    this.txtExtipi.Text = read["mfs_extipi"].ToString();                
                    if (read["mfs_genero"] != DBNull.Value)
                    {
                        this.nudGenero.Value = Convert.ToDecimal(read["mfs_genero"], CultureInfo.InvariantCulture);
                    }

                    this.cbxSobrescreverPIS.Checked = Convert.ToBoolean(Convert.ToInt16(read["mfs_pis"]));
                    if (this.cbxSobrescreverPIS.Checked)
                    {
                        this.cmbCSTPis.SelectedValue = read["mfs_pis_cst"].ToString();
                        this.cmbModTributPis.SelectedValue = read["mfs_pis_modalidade_tributacao"].ToString();
                        this.cbxImpRetidoPis.Checked = Convert.ToBoolean(Convert.ToInt16(read["mfs_pis_imposto_retido"]));
                        if (read["mfs_pis_aliquota"] != DBNull.Value)
                        {
                            this.nudAliquotaPis.Value = Convert.ToDecimal(read["mfs_pis_aliquota"], CultureInfo.InvariantCulture);
                        }
                    }

                    this.cbxSobrescreverCOFINS.Checked = Convert.ToBoolean(Convert.ToInt16(read["mfs_cofins"]));
                    if (this.cbxSobrescreverCOFINS.Checked)
                    {
                        this.cmbCSTCofins.SelectedValue = read["mfs_cofins_cst"].ToString();
                        this.cmbModTributCofins.SelectedValue = read["mfs_cofins_modalidade_tributacao"].ToString();
                        this.cbxImpRetidoCofins.Checked = Convert.ToBoolean(Convert.ToInt16(read["mfs_cofins_imposto_retido"]));
                        if (read["mfs_cofins_aliquota"] != DBNull.Value)
                        {
                            this.nudAliquotaCofins.Value = Convert.ToDecimal(read["mfs_cofins_aliquota"], CultureInfo.InvariantCulture);
                        }
                    }

                    this.cmbModeloICMS.SelectedValue = read["id_modelo_fiscal_icms"].ToString();
                    this.cmbNCM.SelectedValue = read["id_ncm"].ToString();
        ...
        private void Salvar()
        {
            try
            {
                ...
                //RevisaoProdutoClass revisao = null;
                //if (this..idProduto != null)
                //{

                //    RevisaoProdutoForm formJustificativa = new RevisaoProdutoForm("Sr(a). " + this.Usuario.useName + " (" + this.Usuario.useLogin + ") essa operação será registrada como uma revisão do cadastro fiscal do material em seu nome. Você deseja prosseguir?");
                ...
                //    else
                //    {
                //        revisao = new RevisaoProdutoClass(formJustificativa.Justificativa, this.Usuario, this.produto, tipoCadastroProduto.Fiscal);
                //    }
                //}
                ...
                command.CommandText = "DELETE FROM material_fiscal WHERE id_material = " + this.id;
                ...
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_origem", NpgsqlDbType.Integer));
                ...
                command.Parameters[command.Parameters.Count - 1].Value = this.nudAliquotaCofins.Value;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_cofins_modalidade_tributacao", NpgsqlDbType.Smallint));
                command.Parameters[command.Parameters.Count - 1].Value = this.cmbModTributCofins.SelectedValue;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_cofins_imposto_retido", NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = Convert.ToInt16(this.cbxImpRetidoCofins.Checked);
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_pis", NpgsqlDbType.Double));
                ...
                command.Parameters[command.Parameters.Count - 1].Value = this.nudAliquotaPis.Value;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_pis_modalidade_tributacao", NpgsqlDbType.Smallint));
                command.Parameters[command.Parameters.Count - 1].Value = this.cmbModTributPis.SelectedValue;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfs_pis_imposto_retido", NpgsqlDbType.Smallint));
                ...
                command.Parameters[command.Parameters.Count - 1].Value = Convert.ToInt32(this.cmbModeloICMS.SelectedValue);
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("id_ncm", NpgsqlDbType.Integer));
                command.Parameters[command.Parameters.Count - 1].Value = Convert.ToInt32(this.cmbNCM.SelectedValue);
        ...
        private void updadeICMS()
        {
            ...
            try
            {
                ...
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("id_modelo_fiscal_icms",
                ...
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_modalidade_determinacao_bc",
                ...
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_percentual_reducao_bc",
                                                                            NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = this.nudPercReducaoBC.Value;
                ...
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_modalidade_determinicao_bc_st",
                                                                            NpgsqlDbType.Smallint));
                command.Parameters[command.Parameters.Count - 1].Value = this.cmbModalidadeDetBCST.SelectedValue;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_percentual_reducao_bc_st",
                                                                            NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = this.nudPercReducaoBCST.Value;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_percentual_mva_st", NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = this.nudMvaST.Value;
                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_percentual_bc_propria",
                                                                            NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = this.nudPercentuaBCPropria.Value;
                ...
                command.Parameters[command.Parameters.Count - 1].Value = this.nudAliquotaCredito.Value;

                command.Parameters.Add(new IWTPostgreNpgsqlCommandParameter("mfi_percentual_diferimento",
                                                                            NpgsqlDbType.Double));
                command.Parameters[command.Parameters.Count - 1].Value = this.nudPercentualDiferimento.Value;
                ...
                command.CommandText = "SELECT id_estado FROM modelo_fiscal_icms_estado WHERE id_modelo_fiscal_icms = " +
                                      idICMS;
        ...
        private void editarDadosICMS()
        {
            try
            {
                ...
                if (read.HasRows)
                {
                    ...
                    this.nudPercReducaoBC.Value = Convert.ToDecimal(read["mfi_percentual_reducao_bc"]);
                    this.nudPercReducaoBCST.Value = Convert.ToDecimal(read["mfi_percentual_reducao_bc_st"]);
                    this.nudMvaST.Value = Convert.ToDecimal(read["mfi_percentual_mva_st"]);
                    ...
                    this.nudPercentualDiferimento.Value = Convert.ToDecimal(read["mfi_percentual_diferimento"]);
        ...
        private void CadMaterialFiscalForm_Shown(object sender, EventArgs e)
        {
